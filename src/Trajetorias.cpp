...
struct Trajectory
{
    ...
    void load(const string &filePath)
    {
        points.clear();
        ifstream f(filePath);
        if (!f.is_open()) { cerr << "Nao foi possivel abrir " << filePath << endl; return; }
        string line;
        ...
        if (!points.empty()) position = points[0];
    ...
    void update(float deltaTime)
    {
        ...
        int next = (currentPoint + 1) % (int)points.size();
        vec3 from = points[currentPoint];
        vec3 to   = points[next];
        float segLen = length(to - from);
        if (segLen < 0.0001f) { currentPoint = next; t = 0.0f; return; }

        t += (speed / segLen) * deltaTime;
        if (t >= 1.0f)
        {
            t -= 1.0f;
            currentPoint = next;
            next = (currentPoint + 1) % (int)points.size();
...
void main()
{
    ...
    if (keyLightOn == 1)
    {
        ...
        result += diff * keyLightColor * att + ks * spec * keyLightColor;
    }
    ...
    if (fillLightOn == 1)
    {
        ...
        vec3  R    = reflect(-L, N);
        float spec = pow(max(dot(V, R), 0.0), ns);
        result += diff * fillLightColor * att + ks * spec * fillLightColor;
    }
    ...
    if (backLightOn == 1)
    {
        ...
        vec3  R    = reflect(-L, N);
        float spec = pow(max(dot(V, R), 0.0), ns);
        result += diff * backLightColor * att + ks * spec * backLightColor;
    }
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Trajetorias -- Amanda Vidal", nullptr, nullptr);
    ...
    trajectory.load("../assets/trajectory.txt");
    trajectory.speed = 2.0f;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = translate(model, trajectory.position);
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
    ...
    if (action == GLFW_PRESS)
    {
        ...
        if (key == GLFW_KEY_T)
        {
            ...
            cout << "Trajetoria: " << (trajectory.active ? "ATIVADA" : "DESATIVADA") << endl;
        }
    }
